      ...
      <UserLayout>
        <DashboardLayout>
          ...
          {authState.connectionRequests.length != 0 &&
            ...
            <h4>My Network</h4>
            {console.log("connections: ", authState.connectionRequests.connections)}
            {authState.connectionRequests.map((user) => <p>{user.userId._id}</p>)}
            {authState.connectionRequests.filter((connections) => connections.status_accepted === true).map((user, index) => {
              return (
                <div onClick={()=>{
