              ...
              return (
                <div key={post._id} className={styles.singleCard}>
                  <div className={styles.singleCard_profileContainer}>
                    ...
                    />
                    {console.log(
                      `${baseURL}/uploads/${post.userId.profilePicture}`,
                    )}
                    <div>
                      <div onClick={async} style={{display: "flex", gap: "1.2rem", justifyContent: "space-between"}}>
                        <p style={{ fontWeight: "bold" }}>{post.userId.name}</p>
                        { post?.userId?._id === authState?.user?.userId?._id && 
                        <div> <i class="fa-solid fa-trash" style={{height: "1.4rem", color: "red", cursor: "pointer"}}></i></div>}
