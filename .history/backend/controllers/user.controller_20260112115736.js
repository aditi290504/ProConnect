...
const updateProfileData = async (req, res) => {
    ...
    const userProfile = await Profile.findOne({ token: token });
    console.log(userProfile);
    if (!userProfile) {
      return res.status(404).json({ message: "Profile not found" });
    }
    const profileUpdate = await Profile.findOne({ userId: userProfile.userId });
    console.log(userProfile);
...
export { register, login, uploadProfilePicture, userUpdate, getUserAndProfile, updateProfileData };
