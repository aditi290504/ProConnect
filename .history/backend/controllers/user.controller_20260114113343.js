...
const whatAreMyConnections = async(req, res) => {
  ...
  try{
    ...
    const connections = await ConnectionRequest.find({userId: user._id, status_accepted: true}).populate('connectionId', 'name username email profilePicture');
  }catch(err){
...
export { register, login, uploadProfilePicture, userUpdate, getUserAndProfile, updateProfileData, getAllUsers, downloadUserProfile, sendConnectionRequest, 
  getMyConnectionsRequest, acceptConnectionRequest
};
