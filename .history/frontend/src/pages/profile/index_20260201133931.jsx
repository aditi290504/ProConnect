...
export default function ProfilePage() {
  ...
  const updateProfilePicture = async (file) => {
    ...
    const response = await clientServer.post(`${baseURL}/api/users/upload_profile_picture`, 
      formData,{
      headers: {
                ...
                <div style={{ flex: "0.8" }}>
                  ...
                  >
                    <input type="text" className={Styles.nameEdit} value={userProfile.userId.name} onChange={(e) =>{
                      setUserProfile({...userProfile, userId, name: e.target.value}) 
                    }}/>
                    <p style={{ color: "grey" }}>
                ...
                {userProfile?.pastWork?.map((work, index) => {
                  return (
                    <div key={index} className={Styles.workHistoryCard}>
                      <pstyle={{fontWeight: "bold",display: "flex",alignItems: "center",gap: "0.8rem", }}>
